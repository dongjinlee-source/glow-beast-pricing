...
export default function App() {
  ...
  async function doComps() {
    ...
    setStep(2);
    // 타임아웃만 사용 — count 기반 abort 제거 (catch 점프 시 최종 파싱 스킵 버그 방지)
    const ctrl = new AbortController();
    const tid = setTimeout(() => ctrl.abort(), 25000);
    let raw = "";
    try {
      await askClaude(
        `You are a North American beauty market expert with up-to-date retail pricing knowledge.
Output ONLY a raw JSON array. No markdown, no text before or after. Start with [.
Keys: brand, product, price_usd (current Amazon/Sephora retail price as of 2024-2025, NOT discounted), volume_ml (numeric ml equivalent), channel, positioning.
IMPORTANT: price_usd must reflect CURRENT retail prices. Typical ranges: mass market $12-20, mid $20-35, premium $35-60+.`,
        `List 6 real competing products sold in North America matching this product profile:
- Category: ${info.category}
- Key ingredients: ${info.ingredients}
- Our product size: ${info.volume}${info.unit}
- Primary channel: ${info.channel}${info.notes ? `
- Notes: ${info.notes}` : ""}

IMPORTANT: Include products of SIMILAR volume when possible. If volume differs significantly, note it in positioning.
Use CURRENT 2024-2025 retail prices (check Amazon/Sephora actual listings mentally).
Start JSON array now:`,
        t => {
          raw = t;
          // 스트리밍 중 완성된 객체가 생길 때마다 즉시 렌더
          const partial = extractCompleteObjects(t);
          if (partial.length > 0) setComps(partial.map(ensureUrls));
        },
        ctrl.signal
      );
    } catch(e) {
      ...
      // abort든 정상완료든 항상 최종 파싱 실행
      clearTimeout(tid);
      const final = extractCompleteObjects(raw);
      if (final.length > 0) setComps(final.map(ensureUrls));
      else if (raw.length === 0 && !compErr) setCompErr("결과가 없습니다. 다시 시도해주세요.");
      setCompBusy(false);
    }
  }
  ...
  async function doStrats() {
    ...
    try {
      await askClaude(
        `Beauty pricing strategist. Output ONLY raw JSON array, start with [, Korean text fields.`,
        `3 pricing strategies for ${info.category} ${info.volume}${info.unit} entering North America. Competitors avg $${avg}: ${data.slice(0,4).map(c=>`${c.brand} $${c.price_usd}`).join(", ")}. Reply ONLY:
[{"name":"한국어전략명","description":"한국어2문장","suggested_price":0,"rationale":"한국어근거"}]`,
        t => {
          raw = t;
          const partial = extractCompleteObjects(t);
          if (partial.length > 0) setStrats(partial);
        },
        ctrl.signal,
        800
      );
      ...
      clearTimeout(tid);
      const final = extractCompleteObjects(raw);
      if (final.length > 0) setStrats(final);
      else if (raw.length === 0 && !stErr) setStErr("전략 생성 실패. 다시 시도해주세요.");
      setStBusy(false);
    }
  }
      ...
      <div className="shell">
        ...
        {step===2 && (
          <div className="card">
            ...
            {/* Loading header — stays visible even as cards stream in */}
            {compBusy && comps.length === 0 && (
              <div style={{display:"flex",alignItems:"center",gap:12,padding:"20px 0 24px"}}>
                <span className="spin" style={{flexShrink:0}}/>
                <div>
                  <div style={{fontWeight:500,fontSize:14}}>북미 시장 경쟁사 분석 중…</div>
                  <div style={{fontSize:12,color:"var(--muted)",marginTop:3}}>제품이 분석되는 순서대로 나타납니다</div>
                </div>
              </div>
            )}
            {compBusy && comps.length > 0 && comps.length < 6 && (
              <div style={{display:"flex",alignItems:"center",gap:10,padding:"0 0 16px",fontSize:13,color:"var(--muted)"}}>
                    ...
                    <div key={cid(c)} style={{marginBottom:10}}>
                      ...
                      <div className={`citem ${sel.includes(cid(c))?"sel":""}`}
                        ...
                        <div style={{flex:1}}>
                          ...
                          <div className="cmeta">{c.volume} · {c.channel} · {c.positioning}</div>
                      ...
                      <div style={{background:"#fafaf8",border:"1px solid var(--bdr)",borderTop:"none",borderBottomLeftRadius:3,borderBottomRightRadius:3,padding:"7px 14px"}}>
                        <a href={c.amazon_url} target="_blank" rel="noreferrer"
                          style={{fontSize:11,fontWeight:500,color:"#c45500",textDecoration:"none",display:"inline-flex",alignItems:"center",gap:5}}>
                          🛒 Amazon에서 보기 ↗
                        </a>
                      </div>
                    </div>
          ...
          <div className="card">
            ...
            {tab==="strategy" && (
              stErr ? (
                ...
                <>
                  {stBusy && strats.length === 0 && (
                    <div style={{display:"flex",alignItems:"center",gap:12,padding:"16px 0 20px"}}>
                      <span className="spin" style={{flexShrink:0}}/>
                      <div>
                        <div style={{fontWeight:500,fontSize:14}}>포지셔닝 전략 분석 중…</div>
                        <div style={{fontSize:12,color:"var(--muted)",marginTop:3}}>전략이 분석되는 순서대로 나타납니다</div>
                      </div>
                    </div>
                  )}
                  {stBusy && strats.length > 0 && strats.length < 3 && (
                    <div style={{display:"flex",alignItems:"center",gap:10,marginBottom:14,fontSize:13,color:"var(--muted)"}}>
                      <span className="spin spin-sm" style={{flexShrink:0,display:"inline-block"}}/> 분석 중…
                    </div>
                  )}
